...
CREATE TABLE IF NOT EXISTS `customers` (
    ...
    CONSTRAINT `gender_status_check` CHECK (
...
CREATE TABLE IF NOT EXISTS `decors` (
    ...
    FOREIGN KEY (`decor_category_id`) REFERENCES `decor_categories` (`decor_category_id`)
);
...
CREATE TABLE IF NOT EXISTS `products` (
    ...
    CONSTRAINT `prod_status_check` CHECK (
...
CREATE TABLE IF NOT EXISTS `des_products` (
    ...
    `originPrice` DECIMAL(10, 2) NOT NULL,
    `status` VARCHAR(8) NOT NULL,
    CONSTRAINT `des_prod_status_check` CHECK (
        `status` IN ('active', 'inactive')
    ),
...
CREATE TABLE IF NOT EXISTS `orders` (
    ...
    CONSTRAINT `active_status_check` CHECK (
...
FROM
    `des_products` AS `des`
    INNER JOIN `customers` AS `cus` ON `des`.`cus_id` = `cus`.`cus_id`
    INNER JOIN `categories` AS `c` ON `des`.`category_id` = `c`.`category_id`
    INNER JOIN `shapes` AS `sh` ON `des`.`shape_id` = `sh`.`shape_id`
    INNER JOIN `sizes` AS `si` ON `des`.`size_id` = `si`.`size_id`
    INNER JOIN `flavours` AS `f` ON `des`.`flavour_id` = `f`.`flavour_id`;

-- init view orders_view: DONE
CREATE OR REPLACE VIEW `orders_view` AS
SELECT `o`.*, `cus`.`first_name`, `cus`.`last_name`, `cus`.`phone`
FROM
    `orders` AS `o`
    INNER JOIN `customers` AS `cus` ON `o`.`cus_id` = `cus`.`cus_id`;

-- init view order_prod_detail_view: DONE
CREATE OR REPLACE VIEW `order_prod_detail_view` AS
SELECT `o`.*, `p`.`name`, `p`.`image`, `p`.`price`, `p`.`originPrice`, `c`.`type`, `sh`.`shape`, `f`.`flavour`
FROM
...
SELECT `de`.*, `decate`.`type`
FROM
    `decors` AS `de`
    INNER JOIN `decor_categories` AS `decate` ON `de`.`decor_category_id` = `decate`.`decor_category_id`;
