const QUERY_DESIGNED_PRODUCTS = {
    ...
    CREATE_DESIGNED_PRODUCT: "INSERT INTO `des_products` (`cus_id`, `decor_detail_id`, `name`, `category`, `size`, `shape`, `price`) VALUES (?, ?, ?, ?, ?, ?, ?)",
    UPDATE_DESIGNED_PRODUCT: "UPDATE `des_products` SET `cus_id` = ?, `decor_detail_id` = ?, `name` = ?, `category` = ?, `size` = ?, `shape` = ?, `price` = ? WHERE `des_prod_id` = ?",
