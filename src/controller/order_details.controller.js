...
import QUERY_ORDERS_DETAILS from "../query/order_details.query.js";
...
export const getOrder_Details = (req, res) => {
	...
	database.query(QUERY_ORDERS_DETAILS.SELECT_ORDER_DETAILS, (error, results) => {
		if (!results) {
			...
			res.status(HttpStatus.OK.code)
				.send(new Response(HttpStatus.OK.code, HttpStatus.OK.status, `Order_Details retrieved`, results));
		}
	});
};
